		...
		link: function (scope, element, attrs, ngModel) {
				...
				ngModel.$validators.maxsize = function(modelValue, viewValue) {
					...
					for (var i = value.length - 1; i >= 0; i--) {
						if (value[i] && value[i].size && value[i].size > maxsize) {
                            scope.productValidation.imagesMaxSize = false;
							return false;
						}
					}
                    scope.productValidation.imagesMaxSize = true;
                    return true;
				};
			}
			...
			if (attrs.accept) {
				...
				ngModel.$validators.accept = function(modelValue, viewValue) {
					...
					for (var i = value.length - 1; i >= 0; i--) {
						if (value[i] && accept[0].indexOf(value[i].type) === -1) {
                            scope.productValidation.imagesAccept = false;
                            return false;
						}
					}
                    scope.productValidation.imagesAccept = true;
                    return true;
				};
			}
			...
			function updateModelWithFile (event) {
                ...
                scope.productModel.productImages.push(files);
